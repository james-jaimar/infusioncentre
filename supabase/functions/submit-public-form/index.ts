...
function renderFormToPdfHtml(
  ...
  for (const field of schema) {
    if (field.field_type === "section_header") {
      ...
      fieldsHtml += `<tr><td colspan="2" style="background:#e8eff7;padding:10px 14px;font-weight:bold;font-size:14px;color:#1a3a5c;border-bottom:2px solid #3E5B84;">${escapeHtml(currentSection)}</td></tr>`;
      continue;
    }
    ...
    if (val !== undefined && val !== null && val !== "") {
      if (Array.isArray(val)) {
        if (val.length > 0 && typeof val[0] === "object") {
          // Table data (medication_table, vitals_table, etc.)
        ...
        // Object data (substance_table, family_table, vitals_row)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (existingPatient) {
      ...
      // Update ID number / phone if provided and not already set
      ...
      // Also try by ID number if provided
      if (body.respondent_id_number) {
    ...
    // 4. Generate PDF HTML
    const pdfHtml = renderFormToPdfHtml(
      template.name,
    ...
    // 5. Send email to Gayle with the form HTML as the email body
